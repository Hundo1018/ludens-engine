...
def main() raises:
    var s = Suite("gpu_cloth")
    comptime W = 32
    comptime H = 32
    comptime STEPS = 60
    comptime ITERS = 8

    # CPU reference always runs: physical sanity of the cloth itself.
    var cpu = cpu_cloth_run[W, H](STEPS, ITERS, 1.0 / 60.0, 0.05)
    # pinned top row stays put
    s.check(
        abs(Float64(cpu.y[0]) - 2.0) < 1e-6 and abs(Float64(cpu.x[W - 1]) - 0.05 * Float64(W - 1)) < 1e-6,
        "pinned row fixed",
    )
    # the free rows sagged below the pinned row but stayed above the floor
    var sagged = True
    ...
    # constraints keep neighbours near rest length (not exploding)
    ...
    comptime if has_accelerator():
        ...
        s.check(max_d < 1e-3, "CPU/GPU parity (same gather-Jacobi math)")
    else:
