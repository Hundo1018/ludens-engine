...
def main() raises:
    ...
    comptime if has_accelerator():
        # warm-up launch (JIT + context) kept out of the timed runs
        var warm = gpu_cloth_run[64, 64](2, ITERS, 1.0 / 60.0, 0.05)
        _ = warm.x[0]

        var g0 = Int(perf_counter_ns())
        var g1 = gpu_cloth_run[64, 64](STEPS, ITERS, 1.0 / 60.0, 0.05)
        var g0e = Int(perf_counter_ns())
        t.add("GPU 64x64 (4k particles)", 4096, "step", g0e - g0, STEPS)
        _ = g1.x[0]

        var g2s = Int(perf_counter_ns())
        var g2 = gpu_cloth_run[128, 128](STEPS, ITERS, 1.0 / 60.0, 0.05)
        var g2e = Int(perf_counter_ns())
        t.add("GPU 128x128 (16k particles)", 16384, "step", g2e - g2s, STEPS)
        _ = g2.x[0]

        var g3s = Int(perf_counter_ns())
        var g3 = gpu_cloth_run[256, 256](STEPS, ITERS, 1.0 / 60.0, 0.05)
